import {compileToAssessML, compileToHTML, parse, getImageSrc, normalizeVariables, normalizeImages, generateVarValue} from '../../assessml';
...
class PrendusAssessMLTest extends HTMLElement {
    ...
    prepareTests(test: any) {
        test('The parse function should take an arbitrary AssessML string and return a correct AssessML AST', [arbAST], (arbAST: AST) => {
            this.beforeTest();
            const flattenedAst = normalizeImages(normalizeVariables(flattenContentObjects(arbAST)));
